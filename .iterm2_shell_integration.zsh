...
# In iTerm2 v3.3.1 this allows the use of interpolated string \(user.gcloudproject)
# and \(user.kubecontext)
function iterm2_print_user_vars() {
  iterm2_set_user_var kubecontext $(awk -F' ' '/current/{print $2}' ~/.kube/config | cut -d_ -f2)
  iterm2_set_user_var gcloudproject $(cat ~/.config/gcloud/configurations/config_poc | grep project | awk '{print $3}')
}
...
if [[ -o interactive ]]; then
  if [ "${ITERM_ENABLE_SHELL_INTEGRATION_WITH_TMUX-}""$TERM" != "screen" -a "${ITERM_SHELL_INTEGRATION_INSTALLED-}" = "" -a "$TERM" != linux -a "$TERM" != dumb ]; then
    ...
    iterm2_prompt_mark() {
      #printf "\033]133;A\007" don't like this
    }
